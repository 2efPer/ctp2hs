...
#define SPI2(cls, fn, t1)                           \
  void cls::fn(t1 a1, CThostFtdcRspInfoField *a2) { \
    static CThostFtdcRspInfoField zero = {0};       \
    if (this->spi_.ffi##fn) {                       \
      this->spi_.ffi##fn(a1, a2 ? a2 : &zero);      \
    }                                               \
  }
...
#define SPI3(cls, fn)                                         \
  void cls::fn(CThostFtdcRspInfoField *a1, int a2, bool a3) { \
    static CThostFtdcRspInfoField zero = {0};                 \
    if (this->spi_.ffi##fn) {                                 \
      this->spi_.ffi##fn(a1 ? a1 : &zero, a2, a3);            \
    }                                                         \
  }
...
#define SPI4(cls, fn, t1)                                            \
  void cls::fn(t1 a1, CThostFtdcRspInfoField *a2, int a3, bool a4) { \
    static CThostFtdcRspInfoField zero = {0};                        \
    if (this->spi_.ffi##fn) {                                        \
      this->spi_.ffi##fn(a1, a2 ? a2 : &zero, a3, a4);               \
    }                                                                \
  }
...
M0(OnFrontConnected)
M1(OnFrontDisconnected, int)
M1(OnHeartBeatWarning, int)
...
T0(OnFrontConnected);
T1(OnFrontDisconnected, int)
T1(OnHeartBeatWarning, int)
